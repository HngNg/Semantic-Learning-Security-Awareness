...
# for rate in range (1, 10):
    ...
    def criterion_default(x_in, y_in, raw_in):
    ...
    # security awareness loss function
    def criterion_secure(x_in, y_in, raw_in):
        out_tmp1 = nn.CrossEntropyLoss()
        out_tmp2 = nn.MSELoss()
        z_in = mlp_mnist(x_in)
        mse_in = lambda2 * out_tmp2(x_in, raw_in) + 100 *out_tmp2(x_in, raw_in)
        loss_channel = lambda1 * out_tmp1(z_in, y_in) + lambda2 * mse_in
        return loss_channel
    ...
    #         loss = criterion_default(out, label, im)
    #         cr1 = nn.MSELoss()
    #         mse = cr1(out, im)
    ...
    # Eve's CNN
    optimizer = torch.optim.SGD(mlp_encoder_eve.parameters(), 1e-3)
    ...
    for e in range(epoch_len):
        ...
        train_acc = 0
        psnr_aver = 0
        mlp_encoder_eve.train()
        for im, label in train_data:
            im = Variable(im)
            label = Variable(label)

            # forward
            out = mlp_encoder_eve(im)
            out_mnist = mlp_mnist(out)

            loss = criterion_secure(out, label, im)
            cr1 = nn.MSELoss()
            mse = cr1(out, im)
            out_np = out.detach().numpy()
            psnr = 10 * np.log10(np.max(out_np) ** 2 / mse.detach().numpy() ** 2)
